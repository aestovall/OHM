...
circ <- function (slice, area, r_max, voxel) {
  ...
  if (!is.null(circle)) {
    ...
    if (area == FALSE) return(slice) 
    else if (area == TRUE) return(data.frame(x = mean(slice$x),
                                             y = mean(slice$y), 
                                             z = mean(slice$z), 
                                             A = nrow(slice)*voxel^2, 
                                             V = nrow(slice)*voxel^3))
...
OHM<- function (tree, slice_n_min = 10, voxel = 0.02, buff = TRUE, 
                d = 20, area = TRUE, r = TRUE, r_max = 0.01, 
                method = "peel", peel_pct = 20, peel_min = 10,
                correct = TRUE, add_top = FALSE) {
  ...
  if (buff == TRUE) tree_vox_buffer<-buffer(tree_vox, d, voxel) else tree_vox_buffer <- tree_vox
  ...
  if (area == TRUE) {
    ...
    for(i in 1:length(z_list)){
      slice <- subset(tree_vox_buffer, z >= z_list[i] & z <= z_list[i+1])
      if (nrow(slice)<slice_n_min) next 
      else if(circ_err(slice, area, r) < r_max | method == "circle") vol_h[i,] <- circ(slice, area = TRUE, r_max, voxel) 
    ...
    if (correct == FALSE) return(vol_h)
    if (correct == TRUE) return(tcorrect(vol_h, tree_vox, voxel, add_top))
    ...
    for(i in 1:length(z_list)){
      slice <- subset(tree_vox_buffer, z >= z_list[i] & z <= z_list[i+1])
      if (nrow(slice)<slice_n_min) next 
      else if(circ_err(slice) < r_max | method == "circle") datalist[[i]] <- circ(slice, area = FALSE, r_max, voxel)[,1:3]
    ...
    slab<-as.data.frame(do.call(rbind, datalist))
    colnames(slab)<-c("x","y","z")
    return(slab)
  }
...
maxznorm <- function (raw_z) return(max(raw_z)-min(raw_z))

tcorrect <- function (ohm_tree, tree_vox, voxel, add_top = FALSE) {
  
  max_h<-maxznorm(tree_vox$z)
  
  # l_pred<-floor(max_h/voxel)-nrow(ohm_tree)
  
  ohm_tree$z<-ohm_tree$z-min(ohm_tree$z[ohm_tree$V>0])
  ohm_tree<-ohm_tree[ohm_tree$V>=0,]
  
  trunk_taper_m<-data.frame(z = ohm_tree$z, d = ohm_tree$V, z2 = ohm_tree$z^2)
  
  tot_z_range<-seq(min(ohm_tree$z),max_h,by=voxel)
  
  trunk_taper_bin<-aggregate(d~z,data = trunk_taper_m, FUN = "sum")
  tree_vox$z<-voxel*(round(tree_vox$z-min(tree_vox$z),2)/voxel)
  ...
  taper_est <- lm(d ~ -z2 + z, weights = n, data = d_h[!(d_h$d<=0&is.na(d_h$d)),])
  ...
  } else if (add_top == FALSE) ohm_tree_corrected<-trunk_taper_m 
  
  ohm_tree_corrected<-merge.data.frame(ohm_tree,
                                       data.frame(z = ohm_tree_corrected$z[order(ohm_tree_corrected$z)],
                                             Vcor = ohm_tree_corrected$d_cor[order(ohm_tree_corrected$z)]),
                                  by = "z")
  
  ohm_tree_corrected<- data.frame(x = ohm_tree_corrected$x,
                                  y = ohm_tree_corrected$y,
                                  z = ohm_tree_corrected$z,
                                  V = ohm_tree_corrected$V,
                                  Vcor = ohm_tree_corrected$Vcor)
